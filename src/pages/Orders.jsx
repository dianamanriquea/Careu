...
export default function Orders() {
  ...
  function viewOrder(order) {
    setLastOrder({ ...order, createdAt: order.createdAt });
  ...
  return (
    <div style={styles.page}>
      ...
      <main style={styles.main} className="page-main">
        {orders.length === 0 ? (
          <EmptyState onNew={() => navigate('/order-calculator')} />
        ) : (
          <>
                ...
                <table style={styles.table}>
                  ...
                  <tbody>
                    {filtered.map((order, i) => (
                      <OrderRow key={order.orderNumber} order={order} alt={i % 2 !== 0} onView={() => viewOrder(order)} />
                    ))}
                  </tbody>
                </table>
              )}
...
function OrderRow({ order, alt, onView }) {
  ...
  return (
    ...
    >
      <td style={styles.td}>
        <span style={styles.orderBadge}>{order.orderNumber}</span>
      </td>
      ...
      <td style={styles.td}>
        <span style={{ color: '#DC2626', fontWeight: '600' }}>${order.calc.patientOwes.toFixed(2)}</span>
      </td>
...
const HeroIllustration = () => (
  <svg width="180" height="110" viewBox="0 0 180 110" fill="none" className="hide-mobile">
...
const styles = {
  ...
  searchWrap: {
    ...
    padding: '10px 14px', background: '#FFFFFF', width: '300px',
  },
